    ...
    <div class="card bordered z-depth-2" style="margin:0% auto; max-width:400px;">
      <div class="card-header">
        ...
        <div class="card-content">
          <?php
          $id=$_GET['id'];
          $stmt = $admin->ret("SELECT * FROM `resident` where r_id=".$id);
          $row = $stmt->fetch(PDO::FETCH_ASSOC);
          ?>
          <div class="m-b-10">
            <form action="Controller/residentcontroller.php" method="POST" enctype="multipart/form-data">

              <div class="card-title strong pink-text">Login</div>
              <p class="card-title-desc">Forgot Password</p>
            </div>
            <!--  <form class="form-floating" action="Controller/staffcontroller.php"> -->
            <div class="form-group">
              <label for="inputEmail" class="control-label">Email</label>
              <input type="text" name="email" id="useremail" value="<?php echo $row['email'];?>" class="form-control"> </div>
              <input type="hidden" name="r_id" id="useremail" value="<?php echo $row['r_id'];?>" class="form-control"> </div>
              <div class="form-group">
                <label for="inputPassword" class="control-label">Password</label>
                <input type="tel" name="password" id="password" class="form-control" id="inputPassword"> </div>
                <div class="form-group">
                  <!-- <input type="submit" name="add" value="login" class="btn btn-primary"> -->
       ...
       <!--  </div>
        <div class="card-action clearfix">
          <div class"row">
            <div class="col-md-4">
              <a href="index.php">
                <button type="button" class="btn btn-link black-text">Back</button>
              </a>
            </div>
            <div class="col-md-4">
              <button type="button" name="submit" class="btn btn-link black-text">Login</button>  
            ...
            <div class="col-md-4">
              <button id="demo" class="btn btn-link black-text" onclick="clearText()">Clear</button>
            </div>
          </div>
                </form>
